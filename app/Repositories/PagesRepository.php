...
class PagesRepository
{
    ...
    {
        $data = Page::select("id", "can_delete")
            ->with('translations:page_id,title,locale')
        ...
        return Datatables::of($data)
            ...
            ->addColumn("action", function ($item) {
                $return =
                    '<a href="' . route("panel.pages.edit.index", ["id" => $item->id]) . '"
                ...
                if ($item->can_delete) {
                    $return .= '
                                ...
                                data-url="' . route("panel.pages.delete", ["id" => $item->id]) . '"
    ...
    public function store($request)
    {
        ...
        Page::updateOrCreate(['id' => 0], $request->all())->createTranslation($request);
    ...
    {
        Page::updateOrCreate(['id' => $id], $request->all())->createTranslation($request);
    ...
    {
        $item = Page::where('id', $id)->where('can_delete', 1);
        if ($item) {
            $item->delete();
